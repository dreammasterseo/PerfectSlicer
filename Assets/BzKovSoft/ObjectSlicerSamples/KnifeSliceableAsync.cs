...
namespace BzKovSoft.ObjectSlicerSamples
{
	...
	public class KnifeSliceableAsync : MonoBehaviour
	{
		...
		void OnTriggerEnter(Collider other)
		{
			...
			if (knife == null)
				return;
            Vector3 vel = knife.getVelocity();
            //UnityEngine.Debug.Log(vel.magnitude);
           

           /* UnityEngine.Debug.Log(knife.getDif() * 1000);
            if (knife.getDif() *1000 < 0)
            {
                UnityEngine.Debug.Log("Ca passe!");
                return;
            }*/
